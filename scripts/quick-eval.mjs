...
function simulate(a,d, p, trials=8000){
  let attWins=0; for(let i=0;i<trials;i++){ const seed=`q_${a}x${d}_${i}_${randomBytes(4).toString('hex')}`; const rng=rngFromSeed(seed);
    ...
    const eqAdj = (a===d? (p.EQUAL_PENALTY||0) : 0); const attThr = clamp(Math.round(a*p.SCALE + p.ATTACKER_BONUS + (a>d?p.EDGE_BONUS:0) + eqAdj - attPenalty),1,95);
...
const p = { SCALE:2.9, ATTACKER_BONUS:6, CROWD_FACTOR:0.4, TIE_BIAS:0.55, EDGE_BONUS:1 };
const rates = [1,2,3,4,5].map(x=>simulate(5,x,p,6000));
